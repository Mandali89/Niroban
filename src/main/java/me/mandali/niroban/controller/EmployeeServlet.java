...
import me.mandali.niroban.service.EmployeeServiceImpl;
...
public class EmployeeServlet extends HttpServlet {
    ...
    private void addEmployee(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        try {
            ...
            request.getSession().setAttribute("successMessage", "The employee has been added successfully!");

            response.sendRedirect(request.getContextPath() + "/employee?action=list");
        } catch (Exception e) {
            e.printStackTrace();
            request.getSession().setAttribute("errorMessage", "Error adding employee: " + e.getMessage());
    ...
    private void updateEmployee(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        try {
            ...
            request.getSession().setAttribute("successMessage", "The employee has been successfully updated !");
            response.sendRedirect(request.getContextPath() + "/employee?action=list");

        } catch (Exception e) {
            e.printStackTrace();
            request.getSession().setAttribute("errorMessage", "Error updating job offer : " + e.getMessage());
            response.sendRedirect(request.getContextPath() + "/employee?action=edit");
        }
    }
